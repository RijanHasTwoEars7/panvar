...
gene_db <- as.data.frame(read_feather("../../../resources/R_resources/G2g_ggf_Zmays.feather"))

annotation_db <- as.data.frame(read_feather("../../../resources/R_resources/Zmays_493_RefGen_V4.annotation_info.feather"))

##If there is a trailing transcript identifier/genome version that needs to be removed
##Setaria = ".v1.1"

transcript_trailer_to_remove <- "\\.[1-9]$"

##Transcript Identifer - For grabbing individual genes from transcript IDs ## '$' denotes trailing number
##Setaria = "\\.[1-9]$"

individual_transcript_identifier <- "\\_T[0-9][0-9][0-9]$"

##Plot specific names
population_name <- "Maize GB panel"
...
single_gene_to_model <- function(gene_id) {
  ...
  exons_bounds <- subset(gene_db, Type == "exon" & GeneID == target_gene)
...
if (length(args)==1) {
  ...
  #phenotype <- read.table("../../../../gregtraits/file_to_run/complete_pairs/group1/D10_lineBLUP_Pheno.txt", header = T, sep = "\t")
  phenotype <- phenotype[!is.na(phenotype[,2]),]
  ...
  for(single_snp_target in snp_targets){
    ...
    pre_mutation_db_IDs <- as.array(unlist(unique(gene_db$GeneID)))
    pre_mutation_db$ID
    pre_mutation_db <- pre_mutation_db %>% filter(ID %in% pre_mutation_db_IDs)
    ...
    pre_mutation_db <- subset(pre_mutation_db, PER_ALT >= 5)

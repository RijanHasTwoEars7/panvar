...
annotation_db <- as.data.frame(read_feather("../../resources/R_resources/Sbicolor_313_v3.1.annotation_file.feather"))
...
snp_targets <- list.files(path=".", "impactful.txt")
...
if (length(args)==1) {
  ...
  for(single_snp_target in snp_targets){
    ...
    diversity_pdfs_specific <- paste("diversity_pdfs/", sep = "")
    diversity_tables_specific <- paste("summary_tables/", sep = "")
    ...
    output_summary_name_with_path <- paste("summary_tables/", single_snp_target, "_summary_output.txt", sep = "")
    ...
    output_sig_name_with_path <- paste("summary_tables/", single_snp_target, "_TukeyLSD_significance_values.txt", sep = "")
...
all_summary_tables_list <- list.files(path = "summary_tables/.", pattern = "summary_output.txt")
all_sig_tables_list <- list.files(path = "summary_tables/.", pattern = "significance_values.txt")
...
for( summary_table_single in all_summary_tables_list){
  ...
  current_table <- read.table(paste("summary_tables/", summary_table_single, sep=""), header = T, sep = "\t")

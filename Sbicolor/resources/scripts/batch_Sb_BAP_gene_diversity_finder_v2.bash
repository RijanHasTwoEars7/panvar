...
mkdir -p ${FOLDER_NAME}
mkdir -p ${FOLDER_NAME}/individual_files_impactful
mkdir -p ${FOLDER_NAME}/individual_files_full

##Remove multiple/non-relvant gene snps

grep -v "-" temp_${GENE_ID}_diversity_output.txt > ${FOLDER_NAME}/individual_files_full/${GENE_ID}_diversity_output.txt 
...
sed -i 's|0\/0|0|g; s|0\/1|1|g; s|1\/1|2|g; s|.\/.|NA|g' ${FOLDER_NAME}/individual_files_full/${GENE_ID}_diversity_output.txt 

##For filtering only on meaningful variants

grep -E 'CHROM|LOW|MODERATE|HIGH' ${FOLDER_NAME}/individual_files_full/${GENE_ID}_diversity_output.txt > ${FOLDER_NAME}/individual_files_impactful/${GENE_ID}_diversity_output_impactful.txt
...
cat resources/vcf.header > ${FOLDER_NAME}/${OUTPUT_NAME}.txt
for file in ${FOLDER_NAME}/individual_files_full/*.txt; do
grep -v CHROM ${file} >> ${FOLDER_NAME}/unsorted_${OUTPUT_NAME}.txt
done
sort -k1,1 -k2,2 >>${FOLDER_NAME}/${OUTPUT_NAME}.txt <${FOLDER_NAME}/unsorted_${OUTPUT_NAME}.txt

#Write file for merged meaningful mutations

grep -E 'CHROM|LOW|MODERATE|HIGH' ${FOLDER_NAME}/${OUTPUT_NAME}.txt > ${FOLDER_NAME}/${OUTPUT_NAME}_impactful.txt

#Clean up 
rm ${FOLDER_NAME}/unsorted_${OUTPUT_NAME}.txt

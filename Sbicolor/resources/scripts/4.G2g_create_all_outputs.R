...
##Load resource files
gene_db <- read.table("../../../resources/R_resources/Sbicolor_313_v3.1.gene.txt", sep = "\t", header = T)


##Read in calls
PAV_Calls <- read.table("../../../resources/R_resources/BAP_PAV_all_calls_hapmap_for_pie_sb_only.txt", header = T, sep = "\t")

##Read in subpopulations calls
PAV_subpops <- read.csv("../../../resources/R_resources/BAP_pop_assignments_q8.csv", header = T)
...
make_gene_mutation_table <- function(gene_id){
  ...
  table <- tableGrob(mutation_table_db)
...
#all_summary_tables_list <- list.files(path = "../../outputs/summary_tables/.")
...
#for( summary_table_single in all_summary_tables_list){
 ...
 # master_table <- rbind(master_table, current_table, fill = T)
#}

#sorghum_anno <- read.csv("../../../resources/sorghum/R_resources/Sbicolor_454_v3.1.1.annotation_info.csv", header = T)
#final_output <- merge(master_table, sorghum_anno, by.x = "Gene_ID", by.y = "locusName", all.x = T, all.y = F)
#write.csv(master_table, "../../outputs/summary_tables/All_queries_gene_summary_table.txt", row.names = F)

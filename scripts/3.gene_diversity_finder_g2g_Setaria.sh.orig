...
if grep -Fq "$GENE_ID" resources/Setaria_viridis_gene_locations/Sviridis_311_v1.1.gene_locations.txt
then

##Get relevant info for gene of interest

gene_info=$(grep ${GENE_ID} resources/Setaria_viridis_gene_locations/Sviridis_311_v1.1.gene_locations.txt)
...
tabix -h resources/Setaria_viridis_VCF_files/Setaria_viridis_1.1.snpEff.vcf.gz ${gene_cord} | grep -E "#|${GENE_ID}" >  ${FOLDER_NAME}/results/gene_hits/$OUTPUT_NAME/${GENE_ID}_temp.vcf
...
java -jar resources/snpEff/SnpSift.jar extractFields - CHROM POS "ANN[*].FEATUREID" REF ALT "ANN[*].EFFECT" "ANN[*].AA" "ANN[*].IMPACT" "GEN[*].GT" | \
grep Chr >>   ${FOLDER_NAME}/results/gene_hits/$OUTPUT_NAME/temp_${GENE_ID}_diversity_output.txt

...
##Load libraries
#LibraryCheckerInstaller(c("ggplot2", "dplyr", "reshape2", "tidyr", "gtable", "patchwork", "stringr", "gridExtra", "grid", "feather"))
...
#gene_db <- as.data.frame(read_feather("../../resources/R_resources/Sbicolor_313_v3.1.gene.gff.feather"))

#annotation_db <- as.data.frame(read_feather("../../resources/R_resources/Sbicolor_454_v3.1.1.annotation.feather"))
...
PAV_subpops <- read.csv("../../resources/R_resources/Setaria_TB_pop_assignment.csv")
PAV_subpops$Subpopulation <- as.factor(PAV_subpops$Subpopulation)

PAV_Calls <- as.data.frame(read_feather("../../resources/R_resources/SV_PAVMatrix_updated.feather"))
...
snp_targets <- list.files(path=".", pattern = "impactful.txt", recursive = T, full.names = F)
...
if (length(args)==9) {
  ...
  ##read in phenotype file
	phenotype_file_name <- args[7]
	working_dir <- args[8]
	phenotype_file <- paste(working_dir, phenotype_file_name, sep = "/")
	project_name <- args[9]
	print(phenotype_file)
 #path_to_pheno_file <- paste("/shares/tmockler_share/private/Data/G2g/universal_species/Avg_BAP_d13C.txt", phenotype_file, sep="")
 ...
 #process all snps with phenotype info
  for(single_snp_target in snp_targets){
    ...
    path_to_target <- paste(single_snp_target, sep="")
    single_snp_target <- gsub(single_snp_target, pattern = ".txt", replacement = "")
    
    ##make folders for each snp_target
   	output.dir.create.name <- paste("../../outputs/", sep = "")
	diversity_pdfs_specific <- paste("../../outputs/diversity_pdfs/", sep = "")
	diversity_tables_specific <- paste("../../outputs/summary_tables/", sep = "")
    
	dir.create(output.dir.create.name, showWarnings = F)
	dir.create(diversity_pdfs_specific, showWarnings = F)
	dir.create(diversity_tables_specific, showWarnings = F)
    ...
    for(gene in genes_of_interest){
      effects_of_interest <- c("HIGH", "MODERATE")
      pheno_geno_table_sig <- subset(pre_mutation_db, EFFECT %in% effects_of_interest & ID == gene)
	...
	sample.out.name <- gsub(".*/","",single_snp_target)
	output_summary_name_with_path <- paste("../../outputs/summary_tables/", sample.out.name, "_summary_output.txt", sep = "")
	write.table(odat, output_summary_name_with_path, sep = "\t", row.names = F)
    
    ##Write out full significance table
	variant_sig_db <- unique.data.frame(variant_sig_db)
	
	output_sig_name_with_path <- paste("../../outputs/summary_tables/", sample.out.name, "_TukeyLSD_significance_values.txt", sep = "")
...
if( length(all_sig_tables_list) >= 1){
...
for( summary_table_single in all_sig_tables_list){
  ...
  write.csv(master_sig_table, "../../outputs/All_queries_gene_pheno_significance.csv", row.names = F)
}
...
##write final outputs
final_output <- merge(master_table, annotation_db, by.x = "Gene_ID", by.y = 1, all.x = T, all.y = F)
write.csv(final_output, "../../outputs/All_queries_gene_summary_table.csv", row.names = F)
